...
! debug Dahvyd Wing 3/9/2020 add wfcU projection of a single wavefunction
! I am doing this so that I do not have to set lda_plus_u = .true. and use
! all the ldaU machinery. I based this off of the vhspi subroutine
subroutine vhpsi_constr (ldap, np, mps, psip, hpsi)
  !-----------------------------------------------------------------------
  ! this routine calculates h|psi> = lambda*|wfcU> <wfcU| psi> + h|psi>
  USE io_global,            ONLY : stdout
  USE kinds,     ONLY : DP
  USE becmod,    ONLY : bec_type, calbec, allocate_bec_type, deallocate_bec_type
  USE ldaU,      ONLY :  nwfcU, wfcU, lambda_wann_constr
  USE control_flags, ONLY : gamma_only
  !
  implicit none
  !
  ...
  IF  (.NOT. gamma_only) THEN
     !!!!! add warning
  END IF
  !!DW 3.3.20 calculate h|psi> = lambda*|wfcU> <wfcU| psi> + h|psi> TODO add indices to wfcU and proj%r
  CALL DGER(2*np, mps, lambda_wann_constr, wfcU, 1, proj%r, 1, hpsi, 2*ldap )

  CALL deallocate_bec_type (proj)
  !
  CALL stop_clock('vhpsi')
  RETURN
end subroutine vhpsi_constr
! end debug
